...
export default function DoctorDetailPage() {
  ...
  const t = useTranslations("doctor_detail")
    ...
    supabase
      ...
      .then(({ data }) => {
        ...
        setDoctor({
          ...
          experience_details:
            data.translations?.[lang]?.experience_details ?? data.experience_details,
        })
        setLoading(false)
      })
  }, [lang, slug])
      ...
      <section className="max-w-7xl mx-auto px-6 py-16 grid lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Stat label={t("stat_experience")} value={`${doctor.experience_years}+ ${t("stat_years")}`} />
            <Stat label={t("stat_expertise")} value={doctor.expertise?.length ?? 0} />
            <Stat label={t("stat_procedures")} value={doctor.procedures?.length ?? 0} />
            <Stat label={t("stat_languages")} value={doctor.languages?.length ?? 0} />
          </div>
            ...
            <section className="bg-card rounded-xl border p-6">
              ...
              <div className="space-y-6">
                {doctor.experience_details.map((e, i) => (
                  <div key={i} className="border-l-4 border-emerald-600 pl-4">
                ...
                {(Array.isArray(doctor.education)
                  ...
                  : (() => {
                      try {
                        ...
                        return Array.isArray(parsed) ? parsed : doctor.education.split(",")
                      } catch {
            ...
            <section className="bg-card rounded-xl border p-6">
              ...
              <div className="flex flex-wrap gap-2">
                {doctor.languages.map((l) => (
                  <span key={l} className="bg-muted px-3 py-1 rounded-full text-sm">
                    {l}
                  </span>
                ))}
              </div>
          ...
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-6">
            <h3 className="font-semibold mb-3">
              {t("why_choose")} {doctor.name}?
            </h3>
            <ul className="space-y-2 text-sm">
              <li>✔ {doctor.experience_years}+ {t("why_items.experience")}</li>
              <li>✔ {t("why_items.trusted")}</li>
              <li>✔ {t("why_items.hospitals")}</li>
              <li>✔ {t("why_items.approach")}</li>
            </ul>
          </div>
        </div>
        ...
        <aside className="space-y-6">
          <div className="bg-card rounded-xl shadow p-6">
            <h3 className="font-semibold mb-3">{t("sidebar.why_riayah")}</h3>
            <ul className="space-y-2 text-sm text-muted-foreground">
              {t.raw("sidebar.items").map((item: string, index: number) => (
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-green-700 text-white rounded-xl p-6">
            <h3 className="font-semibold mb-2">{t("sidebar.need_help")}</h3>
            <p className="text-sm mb-4">{t("sidebar.help_desc")}</p>
            <button
