...
export default function AdminBanners() {
  ...
  const fetchBanners = async () => {
    try {
      ...
      console.error("Error fetching banners:", error instanceof Error ? error.message : error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleEdit = (banner: Banner) => {
    ...
    const defaultTranslations = { en: { title: "", subtitle: "", cta_text: "" }, ar: {} }
    const currentTranslations = banner.translations || defaultTranslations

    setFormData({
      image_url: banner.image_url,
      cta_text_en: currentTranslations.en?.cta_text || "Get Quote",
      cta_link: banner.cta_link || "/hospitals",
      is_active: banner.is_active ?? true,
      title_en: currentTranslations.en?.title || "",
      subtitle_en: currentTranslations.en?.subtitle || ""
    })
  }
  ...
  const handleSave = async () => {
    try {
      ...
      setEditingId(null)
      fetchBanners()
      alert("Banner updated successfully! Arabic translation has been applied.")
    } catch (error) {
    ...
    try {
      ...
      const response = await fetch("/api/admin/banners", {
        ...
        body: JSON.stringify({
          ...
          title_en: "New Banner",
          subtitle_en: "Click to edit",
          cta_text_en: "Get Quote"
        })
      })

      const data = await response.json()
      if (!response.ok) throw new Error(data.error || "Failed to create banner")

      await fetchBanners()
      
      if (data.data && data.data.length > 0) {
        const newBanner = data.data[0]
        setEditingId(newBanner.id)
        const newTranslations = newBanner.translations || { en: {} }
        
        setFormData({
          image_url: newBanner.image_url,
          cta_text_en: newTranslations.en?.cta_text || "Get Quote",
          cta_link: newBanner.cta_link || "/hospitals",
          is_active: newBanner.is_active ?? true,
          title_en: newTranslations.en?.title || "",
          subtitle_en: newTranslations.en?.subtitle || ""
        })
      }

      alert("Banner created successfully with Arabic translation!")
    } catch (error) {
  ...
  const getTranslationText = (banner: Banner, key: 'title' | 'subtitle' | 'cta_text') => {
      return banner.translations?.en?.[key] || `[No ${key}]`
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={handleAddNew} disabled={saving} className="btn-glass text-white">
          {saving ? "Creating..." : "+ Add Banner"}
        </Button>
      </div>
            ...
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {banner.translations?.ar?.title && (
                    <p className="text-xs text-green-600 mt-1">✓ Arabic translation available</p>
                  )}
                </div>
                <span
              ...
              {editingId === banner.id ? (
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-gray-600 bg-blue-50 p-3 rounded">
                    ℹ️ Arabic translation will be generated automatically when you save
                  </p>
                  <div className="flex gap-2">
                    <Button onClick={handleSave} className="btn-glass text-white flex-1" disabled={uploading || saving}>
