...
interface Hospital {
  ...
  beds: number | null 
  description: string
  // ⭐ FIX: Changed property name in Interface to match the unqualified name in the working query ⭐
...
export default function HospitalsPage() {
  ...
  useEffect(() => {
    const fetchHospitals = async () => {
      ...
      // Query uses the unqualified name: 'hospital_specialties'
...
`)
      ...
      const fetchedHospitals = (data as Hospital[]) || []

      setHospitals(fetchedHospitals)
      setFilteredHospitals(fetchedHospitals)
      setLoading(false)

      // Collect all unique specialties for the dropdown, safely handling potential nulls
      const specialties = fetchedHospitals.flatMap(h => 
        // ⭐ FIX: Accessing the corrected property name 'hospital_specialties' ⭐
        (h.hospital_specialties || []).map(s => s.specialty).filter(s => s) 
      )
      setAllSpecialties([...new Set(specialties)].sort()) 

      // Get initial specialty from URL after data is fetched
  ...
  useEffect(() => {
    ...
    if (selectedSpecialty) {
      ...
      filtered = filtered.filter((h) => {
        // ⭐ FIX: Accessing the corrected property name 'hospital_specialties' ⭐
        return (h.hospital_specialties || []).some(s => 
          s.specialty && 
  ...
  // Countries are still derived correctly from the base data
                ...
                <Link key={hospital.id} href={`/hospitals/${hospital.id}`}>
                  <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition cursor-pointer overflow-hidden">
                    ...
                    <div className="p-6">
                      ...
                      {/* ⭐ FIX: Accessing the corrected property name 'hospital_specialties' in JSX ⭐ 
                    <p className="text-green-600 font-semibold text-sm mb-3">
                      ...
                      <p className="text-green-600 font-semibold text-sm mb-3">
                        {(hospital.hospital_specialties || [])
                          ...
                          .slice(0, 2)     
                          .join(', ')}
                        {/* Optional: Add an ellipsis or '(...)' if there are more than 4 specialties */}
